...
## فحوصات الحماية المضافة في هذه النسخة

أثناء تشغيل GitHub Actions، لن يتم نشر الموقع إلا إذا تحققت الشروط الآتية:

- وجود `out/index.html`.
- وجود `out/_next/static`.
- وجود `out/.nojekyll`.
- عدم وجود أي رابط قديم يبدأ بـ `/qudurat/_next/`.
- وجود رابط CSS يبدأ مباشرةً بـ `/_next/static/css/`، مع التأكد أن الملف المشار إليه موجود فعلًا داخل مجلد `out`.

هذه الفحوصات تمنع نشر نسخة قد تبدو صحيحة في الصفحة الرئيسية لكنها تفشل في تحميل التنسيق أو JavaScript على النطاق المخصص.
